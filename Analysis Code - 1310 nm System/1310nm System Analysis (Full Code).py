...
# IMPORT .TIF FILE:
im = tf.imread(r'D:\Honours Project\1310 nm System\G3\output_files\G3_220_(2)0_00\Int_flip(2)0.tif')
...
threshStrict = 100 #100   # ~10000 for standard scans, ~250 for normalised scans, ~20000 for Lab 850 scans
threshSmooth = 100 #100   # ""
...
#     line1 = plt.plot(x,surf1,'-', color='red', linewidth=0.5)   # [x,im.shape[2]] for full width
#     plt.ylim(0, im.shape[1])
#     line2 = plt.plot(x,surf2,'-', color='cyan', linewidth=0.5)   # [x,im.shape[2]] for full width
...
#     for m in d_surf1:
#         if m >= 2 in d_surf1[20:im.shape[2]-20]:   # m = gradient threshold (..., -1, -2, 0, 1, 2, ...), im.shape[2]-20 removes 20px of noise from end of image
...
#                 detected = True
#                 break
#         elif m <= -2 in d_surf1[20:im.shape[2]-20]:   # -4 default
#             max_value = max(d_surf1)
#             max_index = d_surf1.index(max_value)
...
# PIXEL-HEIGHT ANALYSIS (TECHNIQUE 1)
    ...
    for m in d_surf1:
        if m >= 2 in d_surf1[20:im.shape[2]-20]:   # gradient threshold (..., -1, -2, 0, 1, 2, ...)
            detected = True
            max_value = max(d_surf1)
            max_index = d_surf1.index(max_value)
            if max_index in np.arange(20,im.shape[2]-20):   # ignores start and end noise
                ...
                break
        elif m <= -2 in d_surf1[20:im.shape[2]-20]: # default 4
            detected = True
            max_value = max(d_surf1)
            max_index = d_surf1.index(max_value)
            if max_index in np.arange(20,im.shape[2]-20):
...
# DUAL-LINE ANALYSIS (TECHNIQUE 2)
    ...
    for b in range(20,im.shape[2]-20): # 0,im.shape[2]
        distance = abs(surf1[b]-surf2[b])
        if distance >= 5: #16 default, 6 for Lab 850 scans
            detected = True
            #print(distance, x[surf1[b]], surf1[b])
            plt.imshow(projection, cmap='gray')
