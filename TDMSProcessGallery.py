...
detected = False   # Starts off with no detections made
x = np.arange(0,680,1)   # Use for 680x512 B-scans
#x = np.arange(0,220,1)   # Use for 220x512 B-scans
...
for i in range(start, finish + 1):   # +1 to include the end of the range in the list
    ...
    surf = af.surfaceDetect(im[i,0:512,0:680],thresh=100,padSize=0,scale=1,buffer=10,skip=5)   # Use for 680x512 B-scans
    #surf = af.surfaceDetect(im[i,0:680,0:512],thresh=100,padSize=0,scale=1,buffer=10,skip=5)   # Use for 220x512 B-scans
    d_surf=[(surf[u+1]-surf[u]) for u in range(len(surf)-1)]   # Determines the adjacent change between two points
    ...
    for m in d_surf:
        if m >= 2 in d_surf[20:660]:   # height-difference threshold (>= 2)
            detected = True   # Detected has been made
            max_value = max(d_surf)
            max_index = d_surf.index(max_value)   # Retrieves the coordinate of the irregularity value 
            count = count+1   # Increases the count measuring the total irregularities detected
            if max_index in np.arange(20,660):   # (20,660) to ignore start and end noise
                plt.plot(max_index, i, marker='+', color='lightgreen')   # Plots a marker on the artwork projection
                break
        elif m <= -2 in d_surf[20:660]:   # height-difference threshold (<= -2)
            detected = True   # Detected has been made
            max_value = max(d_surf)
            max_index = d_surf.index(max_value)   # Retrieves the coordinate of the irregularity value 
            count = count+1   # Increases the count measuring the total irregularities detected
...
for n in range(start, finish + 1):   # +1 to include the end of the range in the list
    x = x - start + 1;   # Make x start at 1
    surf = af.surfaceDetect(im[n,0:512,0:680],thresh=100,padSize=0,scale=1,buffer=10,skip=5)   # Use for 680x512 B-scans
    #surf = af.surfaceDetect(im[n,0:680,0:512],thresh=100,padSize=0,scale=1,buffer=10,skip=5)   # Use for 220x512 B-scans
    plt.figure()
    plt.ylim(250)   # Cuts off excess area from bottom of plot
    plt.imshow(im[n,:,:],cmap='gray')   # Shows the B-scan from the range
    x = np.arange(0,680,1)   # Limits the x-axis to 680
    line = plt.plot(x,surf,'-', color='red', linewidth=0.5)   # Plots the surface line on the B-scan
    plt.title('B-Scan: n, '+ str(n))

    d_surf=[(surf[i+1]-surf[i]) for i in range(len(surf)-1)]   # Determines the adjacent change between two points
    #print(d_surf)
    ...
    for m in d_surf:
        if m >= 2 in d_surf[20:660]:   # height-difference threshold (>= 2)
            ...
            max_index = d_surf.index(max_value)   # Retrieves the coordinate of the irregularity value
            if max_index in np.arange(20,660):   # (20,660) to ignore start and end noise
                plt.plot(x[max_index], surf[max_index], marker='+', color='lightgreen')   # Plots a marker on the irregularity
