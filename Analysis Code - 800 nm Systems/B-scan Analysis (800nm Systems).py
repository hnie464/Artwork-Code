...
threshStrict = 10000   # ~10000 for Lumedica 840 nm scans, ~200 for normalised scans, ~20000 for Lab 850 nm scans
threshSmooth = 10000   # ""
...
for n in range(start, finish):  # Loop prints defect analysed B-scans from selected range.
...
# PIXEL-HEIGHT ANALYSIS (TECHNIQUE 1)
    ...
    for m in d_surf1:
        if m >= 4 in d_surf1[20:im.shape[2]-20]:   # Gradient threshold (m >= ..., -1, -2, 0, 1, 2, ...)
            ...
            if max_index in np.arange(20,im.shape[2]-20):   # Used to eliminate noise detection on left- and right-edges on B-scan.
                ...
                detected = True
                break
        elif m <= -4 in d_surf1[20:im.shape[2]-20]:   # "" (Same as above, but for a decreasing defect).
            max_value = max(d_surf1)
            max_index = d_surf1.index(max_value)
...
# DUAL-LINE ANALYSIS (TECHNIQUE 2)    
    for b in range(20,im.shape[2]-20):
        ...
        if distance >= 16:   # If distance between surface lines exceeds a certain value.
            Detected = True
